"""
  ...
  Receives UDP messages on port 5150 from ELite Dangerous client
        
    if  received contains "Publish" then reply with: "{\"Subscribe\":true,\"MotionBoardData\":true}";
    
    else message contains telemetry data:
        "LinearAcceleration": "x:0.005790,y:0.033458,z:0.000686",
        "AngularAcceleration": "x:0.000000,y:-0.000000,z:0.000013",
        "IsDocked": "1",
        "FSDDeparting": "0",
        "FSDArriving": "0",
        "FSDChargeProp": "0.000000"
...
class InputInterface(object):
    ...
    def __init__(self):
        self.rootTitle = "Elite Dangerous Platform Interface"
        self.HOST = ""
        self.PORT = 5150
    ...
    def service(self):
        ...
        while not self.inQ.empty():
            json_obj = self.inQ.get()
            try:
                if json_obj is not None:
                    # print json_obj
                    xyzrpy=[] 

                    translate =json_obj['LinearAcceleration']
                    td = dict(t.split(":") for t in translate.split(","))
                    rotate = json_obj['AngularAcceleration']
                    rd = dict(r.split(":") for r in rotate.split(","))
                    xyzrpy.append(float(td['z']))
                    xyzrpy.append(float(td['x']))
                    xyzrpy.append(float(td['y']))
                    xyzrpy.append(float(rd['z']))
                    xyzrpy.append(float(rd['x']))
                    xyzrpy.append(float(rd['y']))
                    
                    """
                    xyz = json_obj.get('LinearAcceleration')
                    rpy = json_obj.get('AngularAcceleration')
                    xyzrpy = xyz+rpy
                    """

                    for idx, a in enumerate(xyzrpy):
                        if a  > 0:
                            if a > r[idx]:
                               r[idx] = a
                        elif a <  0:
    ...
    def listener_thread(self, inQ, HOST, PORT):
        ...
        while True:
            try:
                ...
                # msg = msg.rstrip()
                if "Publish" in msg:
                    replyMessage = '{"Subscribe":true,"MotionBoardData":true}'
                    client.sendto(replyMessage, address);
                    print "replied to publish msg"
                else:
                    json_obj = json.loads(msg)
                    self.inQ.put(json_obj)
            except:
                e = sys.exc_info()[0]
                s = traceback.format_exc()
